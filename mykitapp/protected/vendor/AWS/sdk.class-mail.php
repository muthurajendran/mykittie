...
class CFRuntime
{
	...
	public function __construct(array $options = array())
	{
		...
		// Retrieve a credential set from config.inc-pc.php if it exists
		...
		if (!$credentials_provided)
		{
			...
			if (!$this->credentials->default_cache_config)
			{
				...
				throw new CFCredentials_Exception('No credentials were provided. The SDK attempts to retrieve Instance '
					...
					. '"default_cache_config" option to be set in the config.inc-pc.php file or constructor. In order to '
...
// Look for include file in the same directory (e.g. `./config.inc-pc.php`).
if (file_exists(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'config.inc-pc.php'))
{
	include_once dirname(__FILE__) . DIRECTORY_SEPARATOR . 'config.inc-pc.php';
}
// Fallback to `~/.aws/sdk/config.inc-pc.php`
else
{
	...
	$path = DIRECTORY_SEPARATOR . '.aws' . DIRECTORY_SEPARATOR . 'sdk' . DIRECTORY_SEPARATOR . 'config.inc-pc.php';
